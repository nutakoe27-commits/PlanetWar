...
step "Прогон сезона"
# ---------------------------------------------------------------------------
hint "Три часа игры на четырёх ботах: все подсистемы работают вместе."
...
if [ -x "$BIN/pw_season" ]; then
    # Галактика маленькая, а часов больше, чем в игре по умолчанию, и это
    # осознанно: прогон обязан ДОВЕСТИ БОТОВ ДО ВСТРЕЧИ. С захватом
    # по планетам расселение стало вчетверо медленнее, и на двухстах
    # системах за час боты не видят друг друга — вся военная часть игры
    # остаётся непроверенной, а прогон при этом зелёный.
    if "$BIN/pw_season" --systems 60 --hours 3 --check >/tmp/pw_season.log 2>&1; then
        ok "инварианты соблюдены"
        grep -E "^Итог: хеш мира" /tmp/pw_season.log | sed 's/^/       /'
        grep -E "^== 3 ч" -A 4 /tmp/pw_season.log | tail -4 | sed "s/^/       /"
    else
