...
void Server::step() {
    // Сжатия времени у живого сервера нет и быть не может: сжатие меняет
    // сами игровые длительности и годится только для проверок. Ускорение
    // сервера (--speed) — совсем другое: оно заставляет тикать чаще
    // в реальном времени, а осада как длилась четырнадцать игровых часов,
    // так и длится.
    const sim::TickContext context = sim::TickContext::at(tick_);
