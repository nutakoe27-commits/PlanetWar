...
public:
    ...
    struct Entry {
        ...
        uint32_t count = 1;
        /// Сквозной номер. Нужен АНИМАЦИИ: карточка выезжает при появлении
        /// и плавно съезжает вверх, когда гаснет соседняя сверху, а для
        /// этого её надо узнавать между кадрами. По месту в списке узнать
        /// нельзя: место меняется как раз тогда, когда идёт движение,
        /// и карточка «переехала бы» вместе со своим содержимым.
        uint32_t seq = 0;
    };
...
enum class ActionKind : uint8_t {
    ...
    OrderShip,
    /// Выбрать флот с идентификатором `value`. Сбрасывает выделение
    /// группы: щелчок по строке — это «работаю с этим», а не «добавляю».
    SelectFleet,
    /// Добавить отряд `value` к выделению или убрать его оттуда.
    ///
    /// Отдельной кнопкой в строке, а не Ctrl-щелчком: клавиатура
    /// в этой игре — только ускоритель, и любой приказ обязан быть
    /// доступен одной мышью.
    ToggleSelect,
    ...
    ///
    /// Быстрый путь, оставшийся рядом с окном разделения: отцепить
    /// колонизатор от боевого флота — самое частое действие в игре,
    /// и оно обязано стоить одного щелчка. Всё, что сложнее одного
    /// корабля, набирается в окне.
    SplitFleet,

    // --- управление отрядом ---
    //
    // Все они относятся к ВЫБРАННОМУ отряду и несут его номер в `value`:
    // экран не хранит состояния, и приказ обязан сам говорить, кому он.

    /// Сменить стойку отряда `value` на `slot` (sim::Stance).
    SetStance,
    /// Включить или выключить уклонение у отряда `value`; `slot` — 0 или 1.
    SetEvade,
    /// Приписать отряд `value` к планете `planet`.
    AnchorPlanet,
    /// Приписать отряд `value` к текущей системе.
    AnchorSystem,
    /// Отправить отряд `value` к своей приписке.
    GoHome,
    /// Снять маршрут отряда `value`.
    ClearRoute,
    /// Открыть или закрыть окно отряда.
    ToggleSquad,
    /// Набрать в окне разделения `value` кораблей класса `slot`.
    SplitAdjust,
    /// Заготовка набора: `value` — 0 сброс, 1 половина, 2 колонизаторы.
    SplitPreset,
    /// Выделить набранный в окне состав из отряда `value`.
    SplitConfirm,
    /// Влить отряд `value` в выбранный.
    MergeFleet,
...
struct ScreenState {
    ...
    /// Приказ взведён: следующий щелчок по карте — это точка маршрута.
    bool awaitingMoveTarget = false;
    /// Сколько точек набрано в этом заходе.
    ///
    /// ПЕРВЫЙ ЩЕЛЧОК ЗАМЕНЯЕТ МАРШРУТ, КАЖДЫЙ СЛЕДУЮЩИЙ ДОБАВЛЯЕТ. Это
    /// и есть shift-клик из RTS, сделанный без единой клавиши: режим
    /// остаётся взведённым, пока игрок не скажет «готово». Без счётчика
    /// экран не мог бы различить первый щелчок и второй, и маршрут
    /// из трёх точек собирался бы тремя заходами в меню.
    uint32_t routePoints = 0;

    /// ВЫДЕЛЕНИЕ: отряды, которым уйдёт приказ. Пустое означает «только
    /// `fleet`»; когда список не пуст, `fleet` — первый из него и служит
    /// главным: окно отряда, разделение и слияние работают с ним одним,
    /// а маршрут, стойка и приписка уходят всем.
    ///
    /// Разделение группой не имеет смысла: «взять по три корвета из
    /// каждого из пяти отрядов» — это не приказ, а таблица.
    std::vector<uint32_t> selection;

    /// Рамка выделения тянется прямо сейчас, в пикселях экрана.
    /// Рисуется экраном, а не картой: она живёт в экранных координатах
    /// и обязана лежать под панелями, а не под звёздами.
    bool bandActive = false;
    float bandX0 = 0.0f, bandY0 = 0.0f, bandX1 = 0.0f, bandY1 = 0.0f;

    /// Окно отряда открыто: состав, стойка, приписка, слияние.
    bool squadOpen = false;
    /// Что в нём набрано. Живёт в состоянии экрана, а не на сервере:
    /// пока игрок не нажал «Выделить», это ещё не приказ, а намерение.
    sim::Fleet splitTake{};
...
private:
    ...
    ScreenAction planetPanel(Ui& ui, const game::Client& client,
                             const ScreenState& state, float top, float width,
                             float& bottom) const;
    /// Окно выбранного отряда: состав и разделение, стойка, приписка,
    /// слияние. Отдельным ОКНОМ, а не строками в столбце: у отряда
    /// столько же управления, сколько у планеты, и втискивать его
    /// в узкую полосу между списком флотов и заказом кораблей значило бы
    /// сделать нечитаемым и то, и другое.
    ScreenAction squadWindow(Ui& ui, const game::Client& client,
