...
struct Selection {
    ...
    /// ВЫДЕЛЕННАЯ ГРУППА. Пустая означает «только `fleet`».
    ///
    /// Нужна карте, а не только панели: приказ уходит всем выделенным,
    /// и игрок обязан видеть НА КАРТЕ, кто именно его получит. Список,
    /// видимый только в панели слева, заставляет сверять номера вместо
    /// того, чтобы смотреть на карту.
    ///
    /// Указателем, а не вектором: `Selection` собирается каждый кадр,
    /// и заводить ради неё выделение памяти незачем — список уже лежит
    /// в состоянии экрана.
    const uint32_t* group = nullptr;
