...
struct SystemCamera {
    ...
    // нет никакой. В системе из восьми тел это заставляет заново
    // разбираться, куда мы попали, — притом что игрок всего лишь щёлкнул
    // по соседней строке списка.
    //
    // Полсекунды перелёта отвечают на вопрос «откуда куда» самим
    // движением, и заодно показывают систему по дороге: расположение
    // орбит запоминается тем, что по ним пролетели.
    //
    // Сглаживание считает ВИД, а не вызывающий: только вид знает, где
    // находятся тела. Поэтому камера передаётся ему по ссылке — она
    // и есть состояние, а не описание.

    /// Куда камера смотрит сейчас. Заполняется видом.
    float lookX = 0.0f;
    float lookY = 0.0f;
    /// Взгляд уже где-то стоит. Первый кадр обязан встать сразу на цель,
    /// иначе камера начинала бы каждую систему перелётом из её центра.
    bool lookValid = false;

    /// За сколько секунд взгляд перелетает к новому телу. Ноль —
    /// мгновенно: так работают снимок и тест, которым нужен устоявшийся
    /// кадр, а не случайная фаза перелёта.
    float followSeconds = 0.0f;
    /// Шаг кадра в секундах. Без него сглаживание зависело бы от частоты
    /// кадров, и на другой машине камера летела бы с другой скоростью.
    float deltaSeconds = 0.0f;

    /// Часы сцены: сколько секунд идёт вид. Копит их сам вид из шага
    /// кадра. Нужны непрерывным колебаниям, которым не нужна память, —
    /// пульсу выбранной орбиты. Ноль означает «движения нет» и даёт
    /// устоявшийся кадр снимку и тесту.
    float clock = 0.0f;
};
...
public:
    ...
    /// Камера НЕ const: вид сглаживает её взгляд между кадрами (см.
    /// SystemCamera). Всё остальное он по-прежнему только читает.
    void build(const sim::Galaxy& galaxy, const game::WorldView& world, uint32_t system,
               uint32_t empire, SystemCamera& camera, float aspect,
