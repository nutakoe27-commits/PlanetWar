...
/// Что вернула полоса набора.
struct DragResult {
    /// Число после этого кадра. Совпадает с переданным, если полосу
    /// не трогают.
    uint32_t value = 0;
    /// Число изменилось именно в этом кадре.
    bool changed = false;
    bool hovered = false;
    /// Полосу держат мышью. Пока держат, число ходит за курсором,
    /// даже если курсор ушёл с полосы: иначе быстрая протяжка
    /// срывалась бы на первом же выходе за край.
    bool held = false;
};
...
public:
    ...
    /// ПОЛОСА НАБОРА: щелчок или протяжка вдоль неё задаёт целое число
    /// от нуля до `range` включительно.
    ///
    /// Почему протяжка, а не пара кнопок «минус» и «плюс». Разделить флот
    /// из сорока корветов пополам счётчиком — это двадцать щелчков, и это
    /// не преувеличение: ровно так выглядела первая версия окна отряда.
    /// Полоса делает то же одним движением и вдобавок показывает ДОЛЮ —
    /// «беру примерно половину» видно, не читая чисел.
    ///
    /// Насечки рисуются, пока корабль на полосе шире четырёх пикселей.
    /// Дальше они сливаются в штриховку и только мешают: при сотне
    /// корветов точность до одного корабля всё равно ничего не решает,
    /// а для края набора есть «всё» и заготовки.
    DragResult dragBar(uint32_t id, const Rect& r, uint32_t value, uint32_t range,
                       const TextColor& color, bool enabled = true);
