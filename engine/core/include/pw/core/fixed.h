...
public:
    ...
    ///
    /// Нужно там, где число ПОКАЗЫВАЮТ. Ни усечение вниз, ни усечение
    /// к нулю для показа не годятся: приход «минус 0,4 в секунду»
    /// при усечении вниз превращается в «-1», при усечении к нулю
    /// в «0», и обе цифры врут — одна пугает, другая успокаивает.
    ///
    /// Сдвиг, а не деление: у сдвига нет расхождений между платформами
    /// на отрицательных числах, а число обязано совпадать у всех.
    constexpr int64_t roundToInt() const {
        return int64_t((raw_ + (kOneRaw >> 1)) >> Frac);
    }
