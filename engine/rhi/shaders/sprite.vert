...
void main() {
    ...
    // Верх текстуры обязан оказаться вверху ЭКРАНА, а не вверху мира.
    //
    // Мир и интерфейс смотрят в разные стороны: у карты ось Y вверх,
    // у экранных панелей вниз. Одна и та же раскладка координат текстуры
    // не может обслужить оба — в одном из них картинка окажется
    // зеркальной. Первая версия этого не учитывала, и корабли на карте
    // летали вверх ногами, а надписи читались в зеркале.
    //
    // Направление берётся из знака масштаба камеры: он и есть ответ на
    // вопрос «куда на этом экране смотрит ось Y».
    vec2 uv = kUv[gl_VertexIndex];
    if (camera.scale.y < 0.0) uv.y = 1.0 - uv.y;
