...
void systemSiege(World& world, const TickContext& context) {
    ...
    world.each<Planet, Owner, PlanetDefense, SiegeState>(
        [&](Entity entity, Planet& planet, Owner& owner, PlanetDefense& defense,
            ...
            // --- ничья планета: НЕ ЗАНИМАЕТСЯ ФЛОТОМ ВООБЩЕ ---
            //
            // Раньше любой флот, постояв над пустой планетой минуту, делал
            // её своей. Это выглядело безобидно — и обесценивало всю
            // колонизацию: зачем строить дорогой медленный корабль, если
            // то же самое даёт бесплатная стоянка любым корветом.
            //
            // Теперь ничью планету берут ТОЛЬКО высадкой колониста
            // (pw/sim/colony.h). Флот над ней может стоять сколько угодно:
            // он её охраняет, но не присваивает. Расширение стоит денег
            // и времени, и это его единственная цена.
            //
            // Осада ничьей планеты тоже не идёт: оборонять там нечего,
            // и «осаждать пустоту» было бы бессмыслицей в журнале.
