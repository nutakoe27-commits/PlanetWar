        ...
        if (tick % policyEvery == 0) {
            ...
            for (Bot& bot : bots) {
                ...
                if (affordable > 0) {
                    ...
                    // ДВА КОЛОНИСТА В ЗАПАСЕ, пока рядом есть свободное.
                    //
                    // С одним прогон на два часа дал разброс в три с половиной
                    // раза: бот, случайно оказавшийся при деньгах в нужный
                    // момент, разросся до семи систем, остальные застряли
                    // на двух. Причина не в удаче, а в очереди: пока флот
                    // везёт единственного колониста через полгалактики,
                    // верфь строит линкоры, и к возвращению флота денег
                    // на нового колониста уже нет.
                    //
                    // Второй колонист разрывает эту очередь: пока первый
                    // в пути, второй уже строится. Расширение перестаёт
                    // зависеть от того, совпали ли деньги с моментом.
                    if (colonists < 2 && freeLand &&
